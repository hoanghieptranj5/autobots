...
{
    private string WebSite(string x) => $"https://hvdic.thivien.net/whv/{x}";
    ...
    public async Task<HanziFromHvDic> CrawlSingle(string hanzi)
    {
        ...
        var meanings = doc.DocumentNode
            .Descendants("div")
            .Where(x => x.Attributes.Contains("class") && x.Attributes["class"].Value.Contains("hvres-meaning han-clickable"))
            .Select(x => x.InnerText);
